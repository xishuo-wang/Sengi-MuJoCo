...
# ==================== 配置参数 ====================
ENABLE_DATA_VISUALIZATION = False           # 是否开启数据可视化
ENABLE_DATA_LOGGING = False                 # 是否开启数据记录
ENABLE_VIDEO_RECORDING = False              # 是否开启视频录制
TOTAL_TIME = 20

# 加载模型
model = mujoco_py.load_model_from_path(r"D:\Code\Model\Sengi\Sengi-terrain.xml")
...
for name in joint_names:
    ...
    joint_dof_indices.append(dof_addr)
...
# 高层控制参数 - 与CSV数据同步
HIGH_LEVEL_DT = CSV_DT  # 240Hz上层控制
...
print(f"将使用CSV数据控制机器人，共 {csv_data_length} 个数据点")
...
if csv_data_length > 0:
    high_level_target_pos = csv_joint_pos_data[:, 0] * 1.0
    high_level_target_vel = csv_joint_vel_data[:, 0] * 1.0
...
while current_time < TOTAL_TIME:
    ...
    if current_time - last_high_level_time >= HIGH_LEVEL_DT - 1e-9 or current_time == 0:
        ...
        # 使用CSV数据
        if csv_data_index < csv_data_length:
            # 根据当前索引获取CSV数据
            target_pos = csv_joint_pos_data[:, csv_data_index % csv_data_length] * 1.0
            target_vel = csv_joint_vel_data[:, csv_data_index % csv_data_length] * 1.0
            ...
            # 如果CSV数据用完，循环使用
            if csv_data_index >= csv_data_length and csv_data_index % 10 == 0:
                print(f"警告: CSV数据已用完，将从头开始循环 (当前时间: {current_time:.3f}s)")
        else:
            # 如果CSV数据用完，循环使用
            csv_data_index = csv_data_index % csv_data_length
            target_pos = csv_joint_pos_data[:, csv_data_index] * 1.0
            target_vel = csv_joint_vel_data[:, csv_data_index] * 1.0
            data_logger.record_csv_target(current_time, target_pos, target_vel)
            csv_data_index += 1
...
print(f"高层控制频率: {data_logger.high_level_updates/current_time:.1f} Hz (目标: {CSV_CONTROL_FREQ}Hz)")
...
class VisualizationController:
    ...
    def plot_joint(self, ax, joint_idx, times):
        ...
        # 绘制目标位置和实际位置
        line1, = ax.plot(times, target_pos, 'r-', linewidth=1.8, label='期望位置(CSV)', alpha=0.8)
        ...
        # 绘制目标速度和力矩
        line3, = ax_right.plot(times, target_vel, 'c-', linewidth=1.5, label='期望速度(CSV)', alpha=0.7)
