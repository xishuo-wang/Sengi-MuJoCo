...
CSV_PATH = r"D:\Software\测力台软件\Data\pi_10_260522_12.csv"
TIME_START = 3.5
TIME_END = 3.65
...
# 映射CSV列到变量
forceX = df['Fx17'].values    # 左腿X方向力
forceY = df['Fy17'].values    # 左腿Y方向力
forceZ = df['Fz17'].values    # 左腿Z方向力
forceXr = df['Fx23'].values   # 右腿X方向力
forceYr = df['Fy23'].values   # 右腿Y方向力
forceZr = df['Fz23'].values   # 右腿Z方向力

# 创建时间步力数据数组
timeStepForce = np.column_stack([forceX, forceY, forceZ])     # 左腿 [Fx, Fy, Fz]
timeStepForcer = np.column_stack([forceXr, forceYr, forceZr]) # 右腿 [Fx, Fy, Fz]
...
Fz_raw = -timeStepForcer[startpoint:endpoint, 2]  # Fz取反，使方向向上为正

# 使用Fz最大值进行归一化（X轴和Y轴使用相同的归一化因子）
...
print("\n" + "=" * 60)
print("自适应扇形统计信息（以Y轴正半轴为参考）:")
print(f"与Y轴最小夹角: {np.degrees(min_angle_y):.2f}°")
print(f"与Y轴最大夹角: {np.degrees(max_angle_y):.2f}°")
...
print(f"  Y轴范围: [{y_min_plot:.2f}, {y_max_plot:.2f}]")
